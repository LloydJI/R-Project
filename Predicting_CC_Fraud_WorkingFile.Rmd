...
```{r}
#A. Load the dataset
creditcardFraud <- read.csv(file.choose())
creditcardFraud
...
corrplot(r, type="lower", tl.col="red",tl.srt=15)
...
#SMOTE Balanced
train.smote <- SMOTE(train.orig[,-31],train.orig[,31])
...
#ADASYN Balanced
train.adas<-ADAS(train.orig[,-31],train.orig[,31])
...
ggplot(aes(x=reorder(Model,-Precision) , y=Precision), data=model_compare_precision) +
  geom_bar(stat='identity', fill = 'light yellow') +
